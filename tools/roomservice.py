...
# Copyright (C) 2014/2015 RageRom Project
#
...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from RageRom Github (http://github.com/RageRom)." % device

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/RageRom/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/rage_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": repo_name, "revision": "rage" })
    ...
    f = open('.repo/local_manifests/rage_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                print 'RageRom/%s already exists' % (repo_name)
            else:
                print 'updating branch for RageRom/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: RageRom/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "RageRom/%s" % repo_name, "revision": "rage" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/rage.dependencies'
    syncable_repos = []
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("device_") and repo_name.endswith("_" + device):
            ...
            add_to_manifest([{'repository':repo_name,'target_path':repo_path,'branch':'rage'}])
...
print "Repository for %s not found in the RageRom Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/rage_manifest.xml" % device
